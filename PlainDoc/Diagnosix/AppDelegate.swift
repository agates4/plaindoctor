...
extension JSON{
    ...
    mutating func appendIfDictionary(key:Int,json:JSON){
        if var dict = self.dictionary{
            dict[String(key)] = json
            self = JSON(dict)
        }
    }
}
...
extension UIView {
    ...
    func fadeIn(duration: TimeInterval = 0.3, delay: TimeInterval = 0.0)
    {
    ...
    func fadeInOut(duration: TimeInterval = 0.3, delay: TimeInterval = 0.0)
    {
        UIView.animate(withDuration: duration, delay: delay, options: [.curveEaseOut], animations: {
            self.alpha = 1.0
